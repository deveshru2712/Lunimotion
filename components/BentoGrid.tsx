...
export default function BentoGrid() {
  return (
    <div className="font-inter min-h-screen w-full bg-neutral-50">
      <div className="mx-auto h-full w-full max-w-6xl border-x py-10">
        <header className="mx-auto w-xl pt-20 pb-10 text-center">
          <span className="font-medium text-orange-500">Features</span>
          <div className="flex flex-col items-center justify-center gap-5">
            <h2 className="text-3xl font-medium tracking-wide">
              Built for Agentic Intellignece
            </h2>
            <p className="mx-auto w-lg">
              Build, test and deploy AI agents with a powerful visual interface
              designed for technical teams
            </p>
          </div>
        </header>
        <main className="grid grid-cols-2 border-y">
          <div className="col-span-2 grid grid-cols-2 divide-x">
            ...
            <div className="flex h-90">
              <Card>
                ...
                <CardSkeleton className="mask-b-from-50% pb-2">
                  <ModelCard />
                </CardSkeleton>
              </Card>
            </div>
            ...
            <div className="flex h-90">
              <Card>
                ...
                <CardSkeleton className="mask-t-from-70%">
                  <WorkflowComponent />
                </CardSkeleton>
              </Card>
            </div>
...
const Card = ({
  ...
  return (
    <div
      className={cn(
        "flex h-full w-full flex-col gap-1.5 overflow-hidden px-6 pt-4 pb-1",
        className,
      )}
    >
      {children}
...
const CardSkeleton = ({
  ...
  return (
    <div className={cn("h-full w-full px-10 pt-6", className)}>{children}</div>
  );
};
...
const modelData = [
  { id: 1, name: "Claude 4 Opus", status: "unavailable", icon: <Claude /> },
  { id: 2, name: "ChatGPT", status: "connected", icon: <OpenAi /> },
  { id: 3, name: "Llama 3.2", status: "waiting", icon: <Meta /> },
];

const ModelCard = () => {
  return (
    <div className="relative h-full w-full rounded-tl-2xl rounded-tr-2xl border-t bg-white px-3.5 py-2 shadow-[0px_20px_15px_10px_rgba(0,0,0,0.05)]">
      <header className="mb-2 flex items-center justify-start gap-2.5">
      ...
      <main className="pt-1">
        <h2 className="flex items-center justify-start gap-2 pt-1 pb-2 text-sm font-medium">
          <Keyboard size={16} /> All Models
          <span className="rounded-2xl bg-gray-200 px-1 py-0.5 text-xs">
            69,6969
          </span>
        </h2>
    ...
    <div className="absolute top-0 -right-1/5 w-fit -translate-x-1/2 -translate-y-1/3 rounded-md border bg-white px-2 py-1">
      <div className="flex h-full w-full flex-col items-center text-xs">
        ...
        <div className="flex w-full justify-start">
          <div className="w-fit rounded-sm border border-blue-400 bg-blue-200 px-0.5 text-blue-400">
            Connected
          </div>
        </div>
      </div>
...
const workflowdata = [
  { id: 1, role: "assistant", text: "Hello! How can I assist you today?" },
  {
    id: 2,
    role: "user",
    text: "Hey! I need help understanding JavaScript arrays.",
  },
  {
    id: 3,
    role: "assistant",
    text: "Sure thing! Arrays are lists used to store multiple values in a single variable.",
  },
];
...
const WorkflowComponent = () => {
  return (
    <div className="flex h-full w-full flex-col items-center justify-between pb-5">
      <div className="flex flex-col gap-3 mask-b-from-80%">
        {workflowdata.map((elem) =>
          elem.role === "assistant" ? (
            <AiMessage key={elem.id} {...elem} />
          ) : (
            <UserMessage key={elem.id} {...elem} />
          ),
        )}
      </div>
